...
public class Picture extends SimplePicture {
	...
	public void mirrorGull() {
		...
		for (int row = 235; row < 319; row++) {
			...
			for (int col = 237; col < mirrorPoint; col++) {
				count++;
				leftPixel = pixels[row][col];
				rightPixel = pixels[row + 3][mirrorPoint - col + mirrorPoint];
				rightPixel.setColor(leftPixel.getColor());
			}
		}
		System.out.println(count);
	...
	public void copy(Picture fromPic, int fromStartRow, int fromStartCol, int endStartRow, int endStartCol,
		...
		for (int fromRow = fromStartRow, toRow = startRow; fromRow < endStartRow
				&& toRow < toPixels.length; fromRow++, toRow++) {
			for (int fromCol = fromStartCol, toCol = startCol; fromCol < endStartCol
					&& toCol < toPixels[0].length; fromCol++, toCol++) {
				fromPixel = fromPixels[fromRow][fromCol];
				toPixel = toPixels[toRow][toCol];
				toPixel.setColor(fromPixel.getColor());
	...
	public void myCollage()
	{
		...
		blueM.keepOnlyBlue();
		blueM.mirrorVertical();
		this.copy(blueM, 100, 100, 200, 200, 100, 100);
		Picture arch = new Picture("arch.jpg");
		arch.zeroBlue();
		this.copy(arch, 100, 100, 200, 200, 200, 100);
